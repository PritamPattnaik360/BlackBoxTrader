...
LEARNING_RATE = 0.3          # weight given to new evidence vs current value
MIN_OUTCOMES  = 10           # minimum outcomes needed before adapting
...
async def compute_new_params(
    ...
    if sells:
        if sell_acc < 0.48:
            target = cur - 0.03   # more negative = fewer SELLs triggered
            events.append(("threshold_raised",
            ...
            target = cur + 0.01   # less negative = more SELLs allowed
            events.append(("threshold_lowered",
                f"SELL threshold {cur:.2f}→{clamp('sell_signal_threshold', target):.2f} "
    ...
    if sharpe < 0.3 or win_rate < 0.40:
        ...
        ceiling = DEFAULTS["risk_per_trade_pct"] * 1.5
        new_risk = min(cur_risk * 1.05, ceiling)
